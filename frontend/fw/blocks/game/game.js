...
export default class Game extends ComponentBase {
  ...
  updatePlayers(message) {
    ...
    const move = this.playerMovePool.getMove(
      player,
      message.direction,
      message.position
    );
    ...
    this.readyPlayers.forEach((player, index) => {
      const playerLife = new ComponentBase(
        "li",
        {
          ...
          style: `${
            this.username === player.props.id
              ? "background-color:rgba(63, 240, 0, .7);"
              : ""
          }`,
        },
        [`${player.props.id} : ❤️${this.lives[index]}`]
      );
  ...
  updateLives() {
    ...
    list.forEach((playerLi, index) => {
      if (
        ...
        if (
          playerLi.props.className === this.currentPlayer.username &&
        ...
        const newContent = this.leavers.includes(playerLi.props.className)
          ? `${playerLi.props.className} : left`
          : `${playerLi.props.className} : dead`;
        if (playerLi.children[0] !== newContent) {
          playerLi.children = [newContent];
