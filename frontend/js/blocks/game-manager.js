...
import WS from "../websocket/ws-conn.js";
...
export default class GameManager {
  ...
  async launchMenu() {
    ...
    ready
      .then((username) => {
        ...
        this.ws = new WS(
          `ws://localhost:8080/api/ws?username=${this.username}`
        );
        this.ws.onOpen(() => {
  ...
  launchgame() {
    const leaveButton = new ComponentBase("button", { id: "leave-button" }, [
      "Leave Game",
    ]);
    const endButton = new ComponentBase("button", { id: "end-button" }, [
      "End Game",
    ]);
    ...
    endButton.actionListener("click", () =>
  ...
  launchEnd() {
    const leaveButton = new ComponentBase(
      "button",
      { id: "leave-button", className: "end" },
      ["Leave Game"]
    );
    ...
    const restart = new ComponentBase(
      "button",
      { id: "restart-button", className: "end" },
      ["Restart"]
    );
