...
export function checkGround(player) {
  ...
  keys.forEach((key) => {
    ...
    if (
      ...
      !solid.has(blockBorder.type) ||
      (blockBorder.type === "bomb" && player.canEscape)
    )
      return;
    switch (key) {
      case "top":
...
const top = (groundObj, blockBorder, playerBorder, playerSpeed) => {
  if (
    !groundObj.groundUp &&
    blockBorder.borderDown - 12 > playerBorder.up - playerSpeed
  ) {
...
const bottom = (groundObj, blockBorder, playerBorder, playerSpeed) => {
  if (
    !groundObj.groundDown &&
    blockBorder.borderUp < playerBorder.down + playerSpeed
  ) {
...
const left = (groundObj, blockBorder, playerBorder, playerSpeed) => {
  if (
    !groundObj.groundLeft &&
    blockBorder.borderRight > playerBorder.left - playerSpeed
  ) {
...
const right = (groundObj, blockBorder, playerBorder, playerSpeed) => {
  if (
    !groundObj.groundRight &&
    blockBorder.borderLeft < playerBorder.right + playerSpeed
  ) {
...
const corner = (corner, groundObj, blockBorder, playerBorder, playerSpeed) => {
  switch (corner) {
    case "top-right":
      if (
        !groundObj.groundUp &&
        blockBorder.borderLeft < playerBorder.right &&
        blockBorder.borderDown - 12 > playerBorder.up - playerSpeed
      ) {
        groundObj.groundUp = true;
        groundObj.right = !groundObj.groundRight
          ? blockBorder.borderLeft - playerBorder.right
          : groundObj.right;
      }
      if (
        !groundObj.groundRight &&
        blockBorder.borderDown - 12 > playerBorder.up &&
        blockBorder.borderLeft < playerBorder.right + playerSpeed
      ) {
        groundObj.groundRight = true;
        groundObj.up = !groundObj.groundUp
          ? blockBorder.borderDown - 12 - playerBorder.up
          : groundObj.up;
      }
      break;
    case "top-left":
      if (
        !groundObj.groundUp &&
        blockBorder.borderRight > playerBorder.left &&
        blockBorder.borderDown - 12 > playerBorder.up - playerSpeed
      ) {
        groundObj.groundUp = true;
        groundObj.left = !groundObj.groundLeft
          ? blockBorder.borderRight - playerBorder.left
          : groundObj.left;
      }
      if (
    ...
    case "bottom-right":
      ...
      if (
        !groundObj.groundRight &&
        blockBorder.borderUp < playerBorder.down &&
        blockBorder.borderLeft < playerBorder.right + playerSpeed
      ) {
        groundObj.groundRight = true;
        groundObj.down = !groundObj.groundDown
          ? playerBorder.down - blockBorder.borderUp
          : groundObj.down;
      }
      break;
    case "bottom-left":
