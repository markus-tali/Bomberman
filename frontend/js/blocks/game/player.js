...
const ANIMATION_FRAMES = {
  down: Array.from({ length: FRAME_COUNT }, (_, i) => ({
    offsetX: i * FRAME_WIDTH,
    offsetY: 0,
  })), // Face to me
  right: Array.from({ length: FRAME_COUNT }, (_, i) => ({
    offsetX: i * FRAME_WIDTH,
    offsetY: FRAME_WIDTH,
  })), // Right-facing
  left: Array.from({ length: FRAME_COUNT }, (_, i) => ({
    offsetX: i * FRAME_WIDTH,
    offsetY: 2 * FRAME_WIDTH,
  })), // Left-facing
  up: Array.from({ length: FRAME_COUNT }, (_, i) => ({
    offsetX: i * FRAME_WIDTH,
    offsetY: 3 * FRAME_WIDTH,
  })), // Back to me
};
...
export class Player extends ComponentBase {
  ...
  animate(direction) {
    ...
    if (
      this.animationCounter % ANIMATION_FRAME_RATE === 0 ||
...
export class CurrentPlayer extends Player {
  ...
  moveCurrent() {
    ...
    this.parent.bonusMap.forEach((bonus) => {
      if (
        checkTrigger(this, bonus) &&
        bonus.parent.children.length == 1 &&
        this.isAlive
      ) {
        let bonusData = {
          bonus: bonus.bonus,
