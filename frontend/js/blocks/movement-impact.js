...
export function initCrossBlast(posX, posY, map, range) {
  ...
  if (check)
    cross.in.push(
      getBorder(map.children[indexY].children[indexX], indexY, indexX)
    );
...
export function initSquareCollision(posX, posY, map) {
  ...
  const square = {
    "top-left":
      mY && check && mX
        ? getBorder(
            map.children[indexY - 1].children[indexX - 1],
            indexY - 1,
            indexX - 1
          )
        : undefined,
    top:
      mY && check
        ? getBorder(
            map.children[indexY - 1].children[indexX],
            indexY - 1,
            indexX
          )
        : undefined,
    ...
    left:
      mX && check
        ? getBorder(
            map.children[indexY].children[indexX - 1],
            indexY,
            indexX - 1
          )
        : undefined,
